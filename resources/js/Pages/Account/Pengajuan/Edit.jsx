...
export default function CategoryEdit() {
    ...
    // Determine tipe pindah based on existing data
    useEffect(() => {
        if (pengajuan.tipe_pindah) {
            setTipePindah(pengajuan.tipe_pindah);
        } else {
            // Auto-detect based on data
            const isDpcMove = tujuan == provinceID;
            setTipePindah(isDpcMove ? "dpc" : "dpw");
        }

        // Filter cities based on tujuan
        if (pengajuan.tipe_pindah === "dpw" && tujuan) {
            const filtered = cities.filter(city => city.province_id == tujuan);
            setFilteredCities(filtered);
        } else if (pengajuan.tipe_pindah === "dpc") {
            const filtered = cities.filter(city => city.province_id == provinceID);
            setFilteredCities(filtered);
        }
    }, [pengajuan, tujuan, provinceID, cities]);
                                    ...
                                    <div className="col-md-6">
                                        <div className="mb-3">
                                            ...
                                            <select
                                                ...
                                                required
                                            >
                                                <option value="">-- Pilih Tipe Pindah --</option>
                                                <option value="dpw">Pindah DPW (Pindah Provinsi)</option>
                                                <option value="dpc">Pindah DPC (Pindah Kabupaten/Kota dalam Provinsi yang sama)</option>
                                            </select>
                                            {errors.tipe_pindah && (
                                            ...
                                            <div className="mb-3">
                                                ...
                                                {tipePindah === "dpw" ? (
                                                    ...
                                                    <select
                                                        ...
                                                        required
                                                    >
                                                        <option value="">-- Pilih Tujuan DPW --</option>
                                                        {provinces.map((province) => (
                                                            <option
                                                                value={province.id}
                                                                key={province.id}
                                    ...
                                    <div className="col-12">
                                        <div className="d-flex gap-2 justify-content-end">
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    setTipePindah(pengajuan.tipe_pindah || "");
                                                    setTujuan(pengajuan.tujuan_mutasi);
                                                    setTujuandpc(pengajuan.dpc_mutasi);
                                                    // Reset filter cities based on current data
                                                    if (pengajuan.tipe_pindah === "dpw" && pengajuan.tujuan_mutasi) {
                                                        const filtered = cities.filter(city => city.province_id == pengajuan.tujuan_mutasi);
                                                        setFilteredCities(filtered);
                                                    } else if (pengajuan.tipe_pindah === "dpc") {
                                                        const filtered = cities.filter(city => city.province_id == provinceID);
                                                        setFilteredCities(filtered);
