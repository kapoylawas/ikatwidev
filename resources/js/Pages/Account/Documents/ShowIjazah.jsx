...
export default function DocumentShowIjazah() {
    ...
    return (
        <LayoutAccount>
            <Head title="Detail Dokumen Ijazah - IKATWI" />
                    ...
                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                        <div className="d-flex align-items-center">
                            ...
                            <div>
                                <h4 className="mb-0 fw-bold header-main-title">
                                    Dokumen Ijazah Pendidikan
                                </h4>
                                <p className="header-subtitle mb-0 mt-1">
                                    Daftar riwayat ijazah pendidikan tinggi dan transkrip nilai terapis wicara.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="d-flex gap-2 flex-wrap flex-shrink-0">
                            <Link
                                ...
                                className="btn btn-add-doc rounded-pill px-4 py-2 fw-bold shadow-sm"
                            >
                                <i className="fa fa-plus-circle me-1.5"></i>
                                <span>Tambah Ijazah Baru</span>
                            </Link>
                            <Link
                        ...
                        <div className="d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h5 className="mb-0 fw-bold table-header-title">
                                    Daftar Berkas Ijazah &amp; Transkrip
                                </h5>
                                <span className="table-header-sub">
                                    Pendidikan Terakhir Anggota IKATWI
                                </span>
                            </div>
                        </div>
                        <span className="badge-total-pill shadow-sm">
                            Total: <strong>{users.dokumen_ijazah?.data?.length || 0}</strong> Data
                        </span>
                    </div>

                    <div className="card-body p-0">
                        {users.dokumen_ijazah?.data?.length > 0 ? (
                                ...
                                <table className="table table-hover align-middle mb-0 custom-doc-table">
                                    <thead className="custom-thead">
                                        <tr>
                                            ...
                                            <th style={{ width: "18%" }}>UNIVERSITAS</th>
                                            <th style={{ width: "15%" }}>JURUSAN</th>
                                            <th className="text-center" style={{ width: "8%" }}>AKREDITASI</th>
                                            <th className="text-center" style={{ width: "8%" }}>LULUS</th>
                                            <th style={{ width: "12%" }}>NO. IJAZAH</th>
                                            <th className="text-center" style={{ width: "6%" }}>IPK</th>
                                            <th className="text-center" style={{ width: "8%" }}>TRANSKRIP</th>
                                            <th className="text-center" style={{ width: "8%" }}>IJAZAH</th>
                                            <th className="pe-4 text-end" style={{ width: "8%" }}>AKSI</th>
                                        </tr>
                                    </thead>
                                    <tbody className="custom-tbody">
                                        {users.dokumen_ijazah.data.map((ijazah, index) => (
                                            <tr key={ijazah.id || index} className="doc-row">
                                                <td className="ps-4 text-center">
                                                    <span className="table-num-pill">
                                                        {index + 1 + (users.dokumen_ijazah.current_page - 1) * users.dokumen_ijazah.per_page}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span className="badge-jenjang shadow-sm">
                                                        {ijazah.ijazah_akhir || "-"}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="text-center">
                                                    <span className="badge-akreditasi shadow-sm">
                                                        {ijazah.akredetasi || "-"}
                                                    </span>
                                                </td>
                                                <td className="text-center font-monospace fw-bold text-slate-700">
                                                    {ijazah.tahun_lulus || "-"}
                                                </td>
                                                ...
                                                <td className="pe-4 text-end">
                                                    <Delete
                                                        URL={"/account/documents/hapus_ijazah"}
                                                        id={ijazah.id}
                                                    />
                                                </td>
                                            </tr>
                            ...
                            <div className="p-5 text-center empty-state-box">
                                ...
                                <h5 className="fw-bold text-slate-900 mb-2">Belum Ada Dokumen Ijazah</h5>
                                <p className="text-slate-600 small mb-4">
                                    Silakan tambahkan data dan berkas ijazah kelulusan Anda untuk melengkapi arsip keanggotaan.
                                </p>
                                <Link
                                    href={`/account/documents/createIjazah/${users.id}`}
                                    className="btn btn-add-doc rounded-pill px-4 py-2 fw-bold shadow-sm"
                                >
                                    <i className="fa fa-plus-circle me-1.5"></i>
                                    <span>Tambah Ijazah Sekarang</span>
                                </Link>
                            </div>
                        )}
                    </div>
